...
Normally I would have called the ~COLOR~ constant ~RED~ (or ~BLUE~ in the case of the first sketch) but I thought it'd be a simple way to show that they're not conflicting with each other - if they were declared globally the sketches would end up the same color, with one definition of ~COLOR~ overriding the other. This seems like a trivial thing, but it can have worse effects than this and more importantly it creates sometimes unpredictable conflicts that are hard to troubleshoot and tedious to work around.
